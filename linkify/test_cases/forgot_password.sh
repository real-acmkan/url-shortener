...
read -r -p "token (from email, take value from ?token=): " token
...
curl -X POST -c cookie-jar -b cookie-jar http://$host/auth/reset-password -H "Content-Type: application/json" -d '{"password":"password"}'
